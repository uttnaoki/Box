...
void main(int ac,char**av){
  ...
  mult33(m1d,m10,m0d);
  ImageImageProjectionAlpha(out,im2,m1d,.5);
  //ImageWrite(outfilename,out);

  //3枚目
  //greedy,lsq
  mt=MatrixAlloc(N1,N3);
  calcSSDtable(mt,im,x1,N1,im3,x3,N3);

  {
    int nm,match[999][2];
    nm=matchMethod(w,mt,im,x1,N1,im3,x3,N3); // 特徴点の対応付け
  }
  for(l=0;l<30000;l++){
    RANSAC(rndAry);
    for(i=0;i<4;i++){
  ...
  */
  ...
  memset(m1d,sizeof(m1d),0);
  mult33(m1d,m10,m0d);
  ImageImageProjectionAlpha(out,im3,m1d,.5);
  ImageWrite(outfilename,out);
}
