...
int main(int argc, char *argv[]){
  ...
  while(strcmp(recv_buf,"practice")){
    printf("%s\n",recv_buf);
    read(0,send_buf,1000);
    sendbuf();                                   //practice or read
    recvbuf();
    if(strcmp(recv_buf,"追加するファイル名を入力してください")==0){
      printf("%s\n",recv_buf);
      read(0,send_buf,1000);
      sendbuf();
      recvbuf();                                // ok
      send_file_data(send_buf);                 // EOFをsend
      recvbuf();
    }
  }
  recvbuf();                                      //ファイル名を入力してください
  
  while(1){                                        //practice start
    printf("%s\n",recv_buf);
